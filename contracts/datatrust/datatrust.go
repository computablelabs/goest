...
//
// Solidity: event ListingAccessed(bytes32 indexed listing_hash, bytes32 indexed delivery_hash, uint256 amount)
func (_Datatrust *DatatrustFilterer) FilterListingAccessed(opts *bind.FilterOpts, listing_hash [][32]byte, delivery_hash [][32]byte) (*DatatrustListingAccessedIterator, error) {

	var listing_hashRule []interface{}
	for _, listing_hashItem := range listing_hash {
		listing_hashRule = append(listing_hashRule, listing_hashItem)
	}
	var delivery_hashRule []interface{}
	for _, delivery_hashItem := range delivery_hash {
		delivery_hashRule = append(delivery_hashRule, delivery_hashItem)
	}

	logs, sub, err := _Datatrust.contract.FilterLogs(opts, "ListingAccessed", listing_hashRule, delivery_hashRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event ListingAccessed(bytes32 indexed listing_hash, bytes32 indexed delivery_hash, uint256 amount)
func (_Datatrust *DatatrustFilterer) WatchListingAccessed(opts *bind.WatchOpts, sink chan<- *DatatrustListingAccessed, listing_hash [][32]byte, delivery_hash [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _Datatrust.contract.WatchLogs(opts, "ListingAccessed", listing_hashRule, delivery_hashRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
